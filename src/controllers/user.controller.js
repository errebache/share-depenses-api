...
const { getUsers, getUser, deleteUser, updateUser, searchUser } = require("../queries/user.queries");
...
exports.emailLinkVerification = async (req, res, next) => {
 ...
 exports.initResetPassword = async (req, res, next) => {
    try {
      ...
      if (!email) { // validateEmail est une fonction hypothétique
      ...
      const genericResponse = { message: "Si l'email existe, un lien de réinitialisation a été envoyé." };
      if (!user) {
...
exports.resetPasswordToken = async (req, res, next) => {
  ...
  exports.resetPassword = async (req, res, next) => {
    try {
      ...
      if (user.passwordToken !== token || moment() >= moment(user.passwordTokenExpiration)) {
